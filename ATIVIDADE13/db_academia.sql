...
FROM tb_alunos
INNER JOIN tb_treinos ON tb_alunos.aluno_id = tb_treinos.aluno_id;
...
FROM tb_alunos
LEFT JOIN tb_treinos ON tb_alunos.aluno_id = tb_treinos.aluno_id
WHERE tb_treinos.treino_id IS NOT NULL;
...
SELECT COUNT(*) AS total_forca_treinos
FROM tb_treinos
WHERE tipo_treino = 'Forca';

-- 7. Encontrar o nome dos alunos e a quantidade de treinos que cada um realizou
SELECT tb_alunos.nome,
COUNT(tb_treinos.treino_id) AS 'Quantidade de Treinos'
FROM tb_alunos
LEFT JOIN tb_treinos ON tb_alunos.aluno_id = tb_treinos.aluno_id
...
SELECT tipo_treino, AVG(duracao) AS 'Media de Duracao'
FROM tb_treinos
...
INNER JOIN tb_treinos ON tb_alunos.aluno_id = tb_treinos.aluno_id
GROUP BY tb_alunos.nome
HAVING COUNT(tb_treinos.treino_id) >= 2;